import etd from './utils/object_extends.js';
...
class SlsPage {
	...
	constructor(opts) {
		...
		for (var f in this.params) {
			if (this.errors.indexOf(f) != -1) {
				if (!this.params[f]) {
					throw new Error('参数 ' + f + ' 不能为空!');
				}
				if (this.params[f] && typeof this.params[f] !== 'number') {
	...
	setPageInfo(cur) {
		...
		for (var i = this.params.page_start; i <= this.params.page_end; i++) {
			...
			if (i == this.pageInfos.page_cur) {
				tempObj.cur = true;
			}
			this.pageInfos.page_lists.push(tempObj);
		}
